...
import { Search, MapPin, Phone, ShieldCheck, Award, Users, ChevronRight, Percent, ArrowRight } from "lucide-react";
...
export default function Home() {
  ...
  const categories = [
    { name: "Running Shoes", count: 184, color: "from-[#2F3C8F] to-[#1d2558]" },
    { name: "Casual Shoes", count: 142, color: "from-[#1c2456] to-[#0c0f24]" },
    { name: "Sports Shoes", count: 96, color: "from-[#3a4ab1] to-[#242e6f]" },
    { name: "Sandals", count: 54, color: "from-[#4a5ce4] to-[#2F3C8F]" },
    { name: "Apparel", count: 110, color: "from-[#131926] to-[#1e293b]" },
    { name: "Accessories", count: 32, color: "from-[#383d47] to-[#121212]" },
  ];
    ...
    <div className="flex flex-col w-full bg-background min-h-screen">
      ...
      <section className="relative w-full h-[85vh] flex items-center justify-center bg-gradient-to-br from-[#10143a] via-[#1a2254] to-black text-white px-space-4 overflow-hidden rounded-b-hero">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center max-w-3xl gap-space-4">
          ...
          <p className="text-body-large md:text-subtitle text-white/70 max-w-xl leading-relaxed">
            Discover 100% authentic models from Nike, Skechers, Puma, Jockey, and Asics across our physical showrooms in Bhuj.
          </p>
          <div className="flex flex-col sm:flex-row items-center gap-space-4 mt-space-6 w-full max-w-md justify-center">
            <Link
              href="/products"
              className="flex items-center gap-2 justify-center w-full sm:w-auto bg-accent text-accent-foreground font-semibold px-space-8 py-space-3.5 rounded-button shadow-soft-lg hover:bg-accent-hover hover:translate-y-[-1px] transition-all"
            >
              Explore Catalogue
            ...
            <Link
              href="/showrooms"
              className="flex items-center justify-center w-full sm:w-auto border border-white/20 bg-white/5 backdrop-blur-sm text-white font-semibold px-space-8 py-space-3.5 rounded-button hover:bg-white/10 transition-colors"
            >
              Find Showrooms
            </Link>
          </div>
          ...
          <div className="flex items-center gap-space-4">
            ...
            <div>
              ...
              <h2 className="text-section-title font-bold leading-tight">
                Flat 20% OFF on all Nike models at FootCare Mall
              </h2>
            </div>
          </div>
          <Link
      ...
      <PageContainer as="section" className="py-space-8 flex flex-col gap-space-4">
        <div className="flex items-end justify-between">
          ...
          <Link href="/brands" className="text-caption font-semibold text-accent flex items-center gap-1 hover:underline">
            View All Brands
      ...
      {/* 5. New Arrivals (Static Carousel/Grid) */}
      <PageContainer as="section" className="py-space-8 flex flex-col gap-space-4">
        <div className="flex items-end justify-between">
          <div>
            <span className="text-label-small text-accent font-semibold uppercase tracking-wider">
          ...
          <Link href="/products?filter=new" className="text-caption font-semibold text-accent flex items-center gap-1 hover:underline">
            View Catalog
            <ChevronRight className="h-4 w-4" />
          </Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-space-6">
          {MOCK_PRODUCTS.filter(p => p.newArrival).map((prod) => (
            <Link
              key={prod.id}
            ...
            >
              ...
              <div className="relative h-60 w-full bg-slate-100 flex items-center justify-center text-slate-400 select-none overflow-hidden">
                {prod.image ? (
                  ...
                  <img src={prod.image} alt={prod.name} className="object-cover w-full h-full group-hover:scale-[1.02] transition-transform duration-500" />
                ) : (
                  <span className="text-caption font-medium uppercase tracking-widest">{prod.brand} Product Image</span>
                )}
                {prod.discount > 0 && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-space-6">
          {categories.map((cat, idx) => (
            <Link
              ...
              className={`relative overflow-hidden h-40 bg-gradient-to-br ${cat.color} rounded-card p-space-6 flex flex-col justify-between text-white shadow-soft-sm group hover:scale-[1.01] transition-transform`}
            >
              <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
              <div className="relative z-10">
                <h3 className="text-body-large font-bold">
                  {cat.name}
                </h3>
              </div>
              <div className="relative z-10 flex items-center justify-between">
                <span className="text-label-small text-white/80 font-medium">
                  {cat.count} Products
                </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-space-8">
          {MOCK_SHOWROOMS.map((showroom) => (
            ...
            >
              ...
              >
                <h3 className="text-body-large font-bold tracking-tight">
                  {showroom.name.includes("Kick Sports")
                    ? "Nike & Skechers"
                    : showroom.name.includes("Store")
                    ? "Puma & Jockey"
                    : showroom.name.includes("Mall")
                    ? "Asics & Joybean"
                    : showroom.name}
                </h3>
              </div>
        ...
        <PageContainer className="grid grid-cols-1 md:grid-cols-3 gap-space-8">
          <div className="flex flex-col gap-space-2 text-center items-center">
            ...
            <p className="text-caption text-foreground/60 max-w-xs">
              Every single pair and piece of apparel in our digital catalog is sourced directly from original manufacturers.
            </p>
          </div>
          <div className="flex flex-col gap-space-2 text-center items-center">
            <div className="w-12 h-12 bg-accent/10 rounded-full flex items-center justify-center text-accent mb-space-2">
            ...
            <p className="text-caption text-foreground/60 max-w-xs">
              Our showroom representatives are trained to help you discover the exact fitting and style for your needs.
            </p>
          </div>
          <div className="flex flex-col gap-space-2 text-center items-center">
            <div className="w-12 h-12 bg-accent/10 rounded-full flex items-center justify-center text-accent mb-space-2">
            ...
            <p className="text-caption text-foreground/60 max-w-xs">
              Built on local trust, FootCare has been the premium footwear shopping choice for thousands of loyal customers.
            </p>
          </div>
        </PageContainer>
      </section>
